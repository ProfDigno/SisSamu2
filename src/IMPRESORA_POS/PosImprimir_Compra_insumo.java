 ...
 * @author Digno
 */
public class PosImprimir_Compra_insumo {
    ...
    private static String nombre_ticket = "COMPRA INSUMO";

    private void cargar_datos_compra_insumo(Connection conn, int idcompra_insumo) {
        String titulo = "cargar_datos_compra_insumo";
        String sql = "select ci.idcompra_insumo as idcompra,\n"
                ...
                + "(ici.fk_idinsumo_producto||'-'||ici.nombre||'-('||ici.unidad||')') as producto_insumo,\n"
                + "TRIM(to_char(ici.precio,'999G999G999')) as precio,\n"
                + "TRIM(to_char(ici.cantidad,'999999D99')) as cantidad,\n"
                + "TRIM(to_char((ici.cantidad*ici.precio),'999G999G999'))  as subtotal\n"
                + "from compra_insumo ci,usuario u,item_compra_insumo ici\n"
                + "where ci.fk_idusuario=u.idusuario \n"
                + "and ci.idcompra_insumo=ici.fk_idcompra_insumo\n"
                + "and ci.idcompra_insumo=" + idcompra_insumo
                + " order by ici.nombre desc";
        try {
            ...
            while (rs.next()) {
                ...
                iv4_descripcion[tk_iv_fila] = rs.getString("producto_insumo");
                tk_iv_fila++;
            }
        } catch (Exception e) {
