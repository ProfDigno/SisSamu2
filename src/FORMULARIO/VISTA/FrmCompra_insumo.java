 ...
 * @author Digno
 */
public class FrmCompra_insumo extends javax.swing.JInternalFrame {
    ...
    DAO_insumo_producto ipdao = new DAO_insumo_producto();
    insumo_producto insup = new insumo_producto();
    compra_insumo compi = new compra_insumo();
    DAO_compra_insumo cidao = new DAO_compra_insumo();
    BO_compra_insumo ciBO = new BO_compra_insumo();
    DAO_item_compra_insumo icidao = new DAO_item_compra_insumo();
    DAO_caja_detalle cdao = new DAO_caja_detalle();
    usuario usu = new usuario();
    PosImprimir_Compra_insumo poscomp = new PosImprimir_Compra_insumo();
    ...
    String insu_tabla = "insumo_producto ip,insumo_categoria ic,insumo_unidad iu";
    String insu_mostrar = "(ic.nombre||'-'||ip.nombre||'-'||iu.nom_compra) as nombre";
    String insu_buscar = "ip.fk_idinsumo_categoria=ic.idinsumo_categoria\n"
            + "and ip.fk_idinsumo_unidad=iu.idinsumo_unidad "
            + "and ip.activar=true \n"
            + "and concat(ic.nombre||'-'||ip.nombre||'-'||iu.nom_compra)";
...
//    }
    ...
    void reestableser_compra() {
        idcompra_insumo_ultimo = (eveconn.getInt_ultimoID_mas_uno(connLocal, compi.getTabla(), compi.getIdtabla()));
    ...
    void seleccionar_buscar_insumo() {
        idinsumo_producto = eveconn.getInt_Solo_seleccionar_JLista(connLocal, jList_insumo_producto, insu_tabla, insu_buscar, "idinsumo_producto");
        ipdao.cargar_insumo_producto(insup, idinsumo_producto);
        txtbuscar_insumo_producto.setText(insup.getNom_categoria() + "-" + insup.getNombre());
        txtinsumo_unidad.setText(insup.getNom_unidad_compra());
        txtinsumo_pre_com.setText(String.valueOf(insup.getPrecio()));
    ...
    void update_precio_insumo() {
        if (hab_update_precio_insumo) {
            ipdao.update_insumo_producto_precio(connLocal, insup);
    ...
    void cargar_item_insumo() {
        if (validar_carga_item()) {
            ...
            try {
                ...
                insup.setIdinsumo_producto(idinsumo_producto);
                insup.setPrecio(DprecioUni);
    ...
    void cargar_datos_compra() {
        compi.setFk_idusuario(usu.getGlobal_idusuario());
        compi.setObservacion(txtobservacion.getText());
        compi.setTiponota(tiponota);
        compi.setEstado(estado);
        compi.setMonto_compra(monto_compra);
        compi.setIndice(indice_creado);
    }
    ...
    void boton_comfirmar_compra() {
        if (validar_compra()) {
            ...
            if (ciBO.getBoolean_compra_insumo(tblitem_producto, compi)) {
                if (habilitar_editar) {
                    habilitar_editar = false;
                    btnconfirmar_insertar.setText("CONFIRMAR");
                    anular_compra(idcompra_insumo_editar, indice_select);
                }
                poscomp.boton_imprimir_pos_compra_insumo(connLocal, idcompra_insumo_ultimo);
                reestableser_compra();
            }
        }
    }
    ...
    void boton_imprimir_pos() {
        if (!evejt.getBoolean_validar_select(tblcompra_insumo)) {
            int idcompra_insumo = evejt.getInt_select_id(tblcompra_insumo);
            poscomp.boton_imprimir_pos_compra_insumo(connLocal, idcompra_insumo);
        }
    }
    ...
    void cargar_datos_caja_insertar(int idcompra_insumo) {
        ...
        caja.setC10tabla_origen("COMPRA_INS");
        caja.setC11fk_idusuario(usu.getGlobal_idusuario());
        caja.setC12indice(compi.getIndice());
...
//            calcular_subtotal_itemventa();
        ...
        if (evt.getKeyCode() == KeyEvent.VK_UP) {
            if (txtinsumo_cant.getText().trim().length() > 0) {
                String cant_actual = txtinsumo_cant.getText().trim();
//                String Sstoc_actual = "100";//p_txtstock_actual.getText().trim();
                double Icant_actual = Double.parseDouble(cant_actual);
//                double Istock_actual = Double.parseDouble(Sstoc_actual);
//                if (Istock_actual > Icant_actual) {
                    Icant_actual = Icant_actual + 1;
//                    System.out.println("cantidad stock actual:" + Icant_actual);
//                } else {
//                    Icant_actual = Istock_actual;
//                }
...
//            calcular_subtotal_itemventa();
    ...
    private void initComponents() {
        ...
        jLayeredPane1.add(jList_insumo_producto, new org.netbeans.lib.awtextra.AbsoluteConstraints(27, 8, 520, 220));
        ...
        jLayeredPane1.add(jScrollPane1, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 20, 910, 310));
        ...
        panel_base_2Layout.setVerticalGroup(
            ...
            .addGroup(panel_base_2Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addComponent(jTab_producto_ingrediente, javax.swing.GroupLayout.PREFERRED_SIZE, 575, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addContainerGap(javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE))
        );
...
//        evetbl.maximizar_jinternal(this);
    ...
    private javax.swing.JTextField txtbuscar_insumo_producto;
